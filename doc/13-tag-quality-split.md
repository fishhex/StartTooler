...
| 改动 | 内容 |
|---|---|
| **词表拆分** | `ChineseTagVocabulary`（56个）→ `SubjectTagVocabulary`（**18个**星体名称）+ `QualityTagVocabulary`（10个质量） |
...
### 2.1 主体词表 `SubjectTagVocabulary`（**18 个**）

用于 AI `tags` 字段 + 左栏标签分类聚合 + `GetByTagAsync` 筛选。

**用户决策（v0.7 实施期）**：tags 严格只允许返回「星体名称」，砍掉：
- **现象 (6)**：极光、日食、月食、凌日、月掩星、合相 —— 是天文事件不是星体
- **构图/手法 (10)**：广角、窄带、行星摄影、深空摄影、星座、长焦、赤道仪跟踪、行星叠加、月面拼接、全景 —— 是拍摄技法不是星体
- **特殊 (2)**：非天文、未分类 —— 是兜底标记不是星体
- **冗余特定名（以此类推）**：
  - 月亮 phases/views：娥眉月 / 满月 / 月面特写 → 月亮
  - 银河 region：银心 → 银河
  - Messier 别名：M42 / M31 / M45 → 猎户座大星云 / 仙女座星系 / 昴星团
  - 子类归大类：暗星云 / 超新星遗迹 → 星云；国际空间站 → 卫星

非天文内容兜底：`tags:[]` + `quality_tags:[问题]` + `score:0`（不再用 "非天文"/"未分类" 标记）。
